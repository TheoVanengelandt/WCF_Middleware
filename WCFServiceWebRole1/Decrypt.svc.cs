		...
		private string GenerateXml(STC_MSG msg)
		{
			...
			if (msg.data != null)
			{
				...
				for (int fileIndex = 0; fileIndex < msg.data.Length; fileIndex++)
				{
					...
					fileNameAttribute.AppendChild(doc.CreateTextNode(msg.data[fileIndex].ToString()));
					fileNode.AppendChild(fileNameAttribute);

					/*
					XmlNode fileNameAttribute = doc.CreateElement("name");
					fileNameAttribute.AppendChild(doc.CreateTextNode(msg.data[fileIndex].name.ToString()));
					fileNode.AppendChild(fileNameAttribute);

					// Get text prop and value from the object
					XmlNode fileTextAttribute = doc.CreateElement("text");
					fileTextAttribute.AppendChild(doc.CreateTextNode(msg.data[fileIndex].name.ToString()));
